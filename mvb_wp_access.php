 ...
 * on latest version for compatibility reasons.
...
class mvb_WPAccess extends mvb_corePlugin {
    ...
    public function pre_get_posts($query) {
        ...
        foreach ($user_roles as $role) {
            ...
            if (isset($this->restrictions[$role]['categories']) && is_array($this->restrictions[$role]['categories'])) {
                ...
                $r_cats = array_merge($r_cats, $t_cats);
            }
    ...
    public function deactivate() {
        ...
        delete_option(WPACCESS_PREFIX . 'key_params');
    }
    ...
    protected function checkPostAccess($post_id) {
        ...
        while (list($i, $role) = each($user_roles)) {
            ...
            if (isset($r_info['posts']) && is_array($r_info['posts'])) {
                $r = $this->checkExpiration($r_info['posts'][$post_id]);
                if (is_admin()) {
                    $restrict = ($r & WPACCESS_RESTRICT_BACK) ? TRUE : FALSE;
                } else {
                    $restrict = ($r & WPACCESS_RESTRICT_FRONT) ? TRUE : FALSE;
                }
                if (isset($r_info['posts'][$post_id]) && $restrict) {
                    wp_die($restrict_message);
                }
            }
        }
        ...
        foreach ($rests as $role => $data) {
            if (is_array($data['categories'])) {
                foreach ($data['categories'] as $cat_id => &$restrict) {
                    ...
                    if ($r) {
                        $restrict['restrict'] = ($r & WPACCESS_RESTRICT_BACK ? 1 : 0);
                        $restrict['restrict_front'] = ($r & WPACCESS_RESTRICT_FRONT ? 1 : 0);
                    ...
                    foreach ($user_roles as $role) {
                        if (isset($this->restrictions[$role]['categories'][$id])) {
                            if (is_admin()) {
                                ...
                                $allowed = ($this->restrictions[$role]['categories'][$id]['restrict_front'] ? FALSE : TRUE);
                            }
                            break;
                        }
                    }
                ...
                if (is_array($user_roles)) {
                    foreach ($user_roles as $role) {
                        if (isset($this->restrictions[$role]['posts'][$id])) {
                            $r = $this->checkExpiration($this->restrictions[$role]['posts'][$id]);
                            if (is_admin()) {
                                $allowed = ($r & WPACCESS_RESTRICT_BACK) ? FALSE : TRUE;
                            } else {
                                $allowed = ($r & WPACCESS_RESTRICT_FRONT) ? FALSE : TRUE;
                            }
                            break;
                        }
                    }
